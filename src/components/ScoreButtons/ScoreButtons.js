...
const ScoreButtons = (props) => {
    return (
        <div className='ScoreButtons' style={{margin: '30px auto'}}>
            ...
            {(props.currentInnings === 2) && (props.score >= props.target) ? <h3>MATCH OVER! TEAM 2 WINS BY {props.totalWickets - props.wickets} WICKETS!</h3> : null}
            {(props.currentInnings === 2) && ((props.balls === props.totalBalls) || (props.wickets === props.totalWickets)) && (props.score<props.target) ? <h3>MATCH OVER! TEAM 1 WINS BY {props.target - props.score - 1} RUNS!</h3> : null}
            <ButtonGroup>
